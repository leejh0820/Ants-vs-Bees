...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if existing.can_contain(self):
                existing.store_ant(self)          
            elif self.can_contain(existing):
                self.store_ant(existing)          
                place.ant = self                  
            else:
    ...
    def nearest_bee(self):
        ...
        while place is not None and not place.is_hive:
            if distance > self.max_range:
                return None
            if distance >= self.min_range and place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        place = self.place
        will_die = (self.health - amount) <= 0
        total = amount + (self.damage if will_die else 0)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        return (self.ant_contained is None) and (not getattr(other, 'is_container', False))
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not getattr(insect, 'is_waterproof', False):
...
class QueenAnt(ScubaThrower):  
    ...
    def construct(cls, gamestate):
        ...
        """
        if not hasattr(gamestate, 'queen_exists'):
            gamestate.queen_exists = False
        if gamestate.queen_exists:
            return None
        
        ant = super(QueenAnt, cls).construct(gamestate)
        if ant is not None:
            gamestate.queen_exists = True
            ant.is_true_queen = True
        return ant
    ...
    def action(self, gamestate):
        ...
        def maybe_double(ant):
            if ant is None:
                return
            if hasattr(ant, 'damage') and not getattr(ant, 'damage_doubled', False):
                ant.damage *= 2
        ...
        while place is not None:
            if place.ant is not None:
                maybe_double(place.ant)
                if getattr(place.ant, 'is_container', False) and place.ant.ant_contained is not None:
                    maybe_double(place.ant.ant_contained)
            place = place.exit
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if ant is None:
            return False
        return getattr(ant, 'blocks_path', True)
    ...
    def action(self, gamestate):
        ...
        if hasattr(self, 'scared_remaining') and self.scared_remaining > 0:
            if (self.place is not None and
                self.place.entrance is not None and
                not self.place.entrance.is_hive):
                self.move_to(self.place.entrance)
            self.scared_remaining -= 1
            return
    ...
    def scare(self, length):
        ...
        """
        if not hasattr(self, 'scared_ever'):
            self.scared_ever = False
        if not hasattr(self, 'scared_remaining'):
            self.scared_remaining = 0

        if not self.scared_ever:
            self.scared_ever = True
            self.scared_remaining = length
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while place is not None and not place.is_hive:
            ...
            if ant is not None:
                if ant is not self:
                    res[ant] = dist
                if getattr(ant, 'is_container', False) and getattr(ant, 'ant_contained', None) is not None:
                    inner = ant.ant_contained
    ...
    def calculate_damage(self, distance):
        dmg = 2.0 - 0.25 * float(distance) - (1.0 / 16.0) * float(self.insects_shot)
        if dmg < 0:
            return 0
        return dmg
